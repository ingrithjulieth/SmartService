...
$(document).ready(function(){
	...
	$('#filterTipo').change(function(){
		if($(this).val() == 'Item'){
			$('#DivfiltrarItem').css('display','block');
			$('#DivfiltrarItem label').html("Top Item");			
		}else if($(this).val() == 'Habitación'){
			$('#DivfiltrarItem').css('display','block');
			$('#DivfiltrarItem label').html("Top Habitación");
		}else{
			$('#DivfiltrarItem').css('display','none');
		}
	});
	
	$('#generaStadisticas').click(function(){
	
		var datos = [];
		var rangoTiempo = $('#filterTime').val();
		var tipo = $('#filterTipo').val();
		switch(tipo){
			
			case 'Selecciona Una':
				alert('Debes seleccionar un tipo de filtrado');
				break;
			case 'Categoria':
				$.post('loadStadistics', {'rangoTiempo': rangoTiempo}, function(data){	
				$.each(data, function(){
					var array = [];
					array.push(this.nom_tipo_servicio);
					array.push(this.total);
					datos.push(array);
				});
				
				console.log(datos);
				google.setOnLoadCallback(drawChart(datos));
						
				}, "json");	
				break;
			case 'Item':
				var rangoItems = $('#filteritem').val();
				$.post('loadStadisticsItem', {'rangoTiempo': rangoTiempo}, function(data){	
				$.each(data, function(index){
					if(index < rangoItems || rangoItems == 0){
						var array = [];
				...
				break;
			case 'Habitación':
				$.post('loadStadisticsRoom', {'rangoTiempo': rangoTiempo}, function(data){	
				$.each(data, function(){
					var array = [];
					array.push(this.habitacion);
					array.push(this.total);
					datos.push(array);
				});
...
function drawChart(datos) {
        ...
        // Set chart options
        var options = {'title':'Consumos por Categoria',
                       'width':650,
                       'height':400};
